...
class Register_serializer(serializers.ModelSerializer):
    ...
    def create(self,validated_data):
        ...
        user.save()
        jwt_token = jwt.encode({"username":user.username,"email":user.email},settings.JWT_SECRET_KEY).decode('utf-8')
...
class Create_password_reset_token_serializer(serializers.Serializer):
    ...
    def validate(self,data):
        ...
        jwt_token = jwt.encode({"username":user.username},key=settings.JWT_SECRET_KEY).decode('utf-8')
        ...
        print(user.email)
        send_mail(subject='reset password',message='reset password token',recipient_list=[user.email],html_message=html_message,from_email='emailtesting17082000@gmail.com')
        return True
